...
import { ensureInstructorProgramAccess, updateWeekAssignment } from "@/features/instructor/server/instructor-service";
...
export async function PATCH(req: Request, { params }: { params: Promise<{ programSlug: string; weekNumber: string }> }) {
  ...
  const data = await updateWeekAssignment(programSlug, Number(weekNumber), await req.json());
