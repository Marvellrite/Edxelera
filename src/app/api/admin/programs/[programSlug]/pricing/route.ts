...
import { setProgramPricing } from "@/features/admin/server/admin-service";
...
export async function PATCH(req: Request, { params }: { params: Promise<{ programSlug: string }> }) {
  ...
  const data = await setProgramPricing(programSlug, Number(body.amountMinor), body.currency ?? "NGN");
